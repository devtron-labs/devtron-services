...
type InformerImpl struct {
	...
	veleroVslInformerStopper map[int]*informerBean.SharedStopper
}
...
func NewInformerImpl(logger *zap.SugaredLogger,
	...
	return &InformerImpl{
		...
		veleroVslInformerStopper: make(map[int]*informerBean.SharedStopper),
	}
}
...
func (impl *InformerImpl) StartInformerForCluster(clusterInfo *repository.Cluster) error {
	...
	startTime := time.Now()
	defer func() {
		impl.logger.Infow("time taken to start velero vsl informer", "clusterId", clusterInfo.Id, "time", time.Since(startTime))
	}()
	...
	impl.logger.Infow("starting velero vsl informer for cluster", "clusterId", clusterInfo.Id, "clusterName", clusterInfo.ClusterName)
	restConfig := impl.k8sUtil.GetK8sConfigForCluster(clusterInfo)
	vslInformerClient := impl.informerClient.GetSharedInformerClient(resourceBean.VeleroVslResourceType)
	vslInformer, err := vslInformerClient.GetSharedInformer(clusterLabels, impl.appConfig.GetVeleroConfig().GetVeleroNamespace(), restConfig)
	if err != nil {
		impl.logger.Errorw("error in registering velero vsl informer", "err", err, "clusterId", clusterInfo.Id)
		return err
	}
	runnable := func() {
		vslInformer.Run(stopChannel)
		impl.logger.Infow("informer started for velero vsl", "clusterId", clusterInfo.Id, "clusterName", clusterInfo.ClusterName)
	}
...
func (impl *InformerImpl) StopInformerForCluster(clusterId int) error {
	stopper, found := impl.getVeleroVslStopper(clusterId)
	if found {
		stopper.Stop()
		delete(impl.veleroVslInformerStopper, clusterId)
...
func (impl *InformerImpl) StopAll() {
	for _, stopper := range impl.veleroVslInformerStopper {
		stopper.Stop()
	}
}
