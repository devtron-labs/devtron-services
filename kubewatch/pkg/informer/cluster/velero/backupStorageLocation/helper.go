...
func (impl *InformerImpl) getVeleroBslStopper(clusterId int) (*informerBean.SharedStopper, bool) {
	stopper, ok := impl.veleroBslInformerStopper[clusterId]
	if ok {
...
func (impl *InformerImpl) getStoppableClusterIds() []int {
	return maps.Keys(impl.veleroBslInformerStopper)
}
