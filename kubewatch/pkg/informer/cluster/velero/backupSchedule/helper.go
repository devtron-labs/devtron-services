...
func (impl *InformerImpl) getVeleroBackupScheduleStopper(clusterId int) (*informerBean.SharedStopper, bool) {
	stopper, ok := impl.veleroBackupScheduleInformerStopper[clusterId]
	if ok {
...
func (impl *InformerImpl) checkAndGetStopChannel(clusterLabels *informerBean.ClusterLabels) (chan struct{}, error) {
	...
	stopper, ok := impl.getVeleroBackupScheduleStopper(clusterLabels.ClusterId)
	...
	impl.veleroBackupScheduleInformerStopper[clusterLabels.ClusterId] = stopper
	return stopChannel, nil
}
func (impl *InformerImpl) getStoppableClusterIds() []int {
	return maps.Keys(impl.veleroBackupScheduleInformerStopper)
}
