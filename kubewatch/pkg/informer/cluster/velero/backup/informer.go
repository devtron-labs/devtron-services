...
type InformerImpl struct {
	...
	veleroBackupInformerStopper map[int]*informerBean.SharedStopper
}
...
func NewInformerImpl(logger *zap.SugaredLogger,
	...
	return &InformerImpl{
		...
		veleroBackupInformerStopper: make(map[int]*informerBean.SharedStopper),
	}
}
...
func (impl *InformerImpl) StartInformerForCluster(clusterInfo *repository.Cluster) error {
	if impl.appConfig.GetExternalConfig().External {
		impl.logger.Warnw("external mode is enabled, skipping the informer start for velero backup", "cluster", clusterInfo)
		return nil
	}
	startTime := time.Now()
	defer func() {
		impl.logger.Debugw("time taken to start informer for velero backup", "time", time.Since(startTime))
	}()
	...
	backupInformerClient := impl.informerClient.GetSharedInformerClient(resourceBean.VeleroBackupResourceType)
	backupInformer, err := backupInformerClient.GetSharedInformer(clusterLabels, impl.appConfig.GetVeleroNamespace(), restConfig)
	if err != nil {
		impl.logger.Errorw("error in getting velero backup informer", "clusterId", clusterInfo.Id, "err", err)
		return err
	}
	runnable := func() {
		backupInformer.Run(stopChannel)
...
func (impl *InformerImpl) StopInformerForCluster(clusterId int) error {
	stopper, found := impl.getVeleroBackupStopper(clusterId)
	if found {
		stopper.Stop()
		delete(impl.veleroBackupInformerStopper, clusterId)
...
func (impl *InformerImpl) StopAll() {
	for _, stopper := range impl.veleroBackupInformerStopper {
		stopper.Stop()
	}
}
